...
def recommend_book_page(grade_id: str, semester_id: str, subject_id: str = ""):
    ...
    if resp and "code" in resp and resp["code"] == "200":
        data = resp["data"]
        if subject_id and "versionBooks" in data:
            return data["versionBooks"]
        if "subject" in data:
            return data["subject"]
    return []
...
def download_ebook(book_id, save_dir, only_to_pdf=False):
...
def get_books_by_grade(grade_id: str, semester_id: str, base_path: str, only_ebook=False, only_to_pdf=False):
    ...
    for subject in subjects:
        ...
        for v_book in v_books:
            ...
            for book_info in book_infos:
                ...
                download_ebook(b_id, save_dir, only_to_pdf)
                if not only_ebook:
                    download_zip(b_id, save_dir)


def main(base_dir, filter_period=None, filter_grade=None, multi_thread=False, only_ebook=False, only_to_pdf=False):
    ...
    for learning_period in learning_periods:
        ...
        for grade in learning_period["gradeList"]:
            ...
            for semester in semester_list:
                ...
                if not multi_thread:
                    get_books_by_grade(g_id, s_id, base_path, only_ebook, only_to_pdf)
                    continue
                thread = threading.Thread(target=get_books_by_grade,
                                          args=(g_id, s_id, base_path, only_ebook, only_to_pdf))
...
if __name__ == "__main__":
    ...
    main(basedir  # , filter_period=lambda p_name: p_name == "小学"
         ...
         , multi_thread=True, only_ebook=True, only_to_pdf=True)
