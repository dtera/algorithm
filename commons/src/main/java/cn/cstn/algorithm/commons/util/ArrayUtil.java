...
public class ArrayUtil {
  ...
  public static boolean isMultiStageGraph(Set<Integer>[] g) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      access[i] = true;
      for (int j = 0; j < n; j++)
        if (!access[j] && !g[i].contains(j)) if (g[i].equals(g[j])) access[j] = true;
        else return false;
    }
  ...
  public static <T> void coCombination(T[] a, Consumer<Pair<List<T>, List<T>>> consumer) {
    ...
    for (int i = 1; i <= a.length / 2; i++) {
      ...
      CollectionUtil.combinationPair(pls, t -> {
        ...
        if (intersect.isEmpty()) consumer.accept(t);
      });
  ...
  private static <T> void combination(T[] a, int l, int n, List<T> pl, Consumer<List<T>> consumer) {
    ...
    if (n == 1) for (int i = l; i < a.length; i++) {
  ...
  public static <T extends Comparable<T>> void merge(T[] arr, int l, int m, int r) {
    ...
    while (i <= m && j <= r) if (arr[i].compareTo(arr[j]) < 0) t[k++] = arr[i++];
    else t[k++] = arr[j++];
  ...
  public static void merge(int[] arr, int l, int m, int r) {
    ...
    while (i <= m && j <= r) if (arr[i] < arr[j]) t[k++] = arr[i++];
    else t[k++] = arr[j++];
  ...
  private static <T> void radixSort(T[] arr, int begin, int end, int k, Character[] radix) {
    ...
    for (int i = 0; i < k; i++) {
      ...
      for (int j = begin; j < end; j++) {
        ...
        if (list == null) list = lists[indexOfCi] = new ArrayList<>();
        list.add(arr[j]);
      }
  ...
  private static <T extends Comparable<T>> void shift(T[] arr, int i, int end) {
    ...
    if (l > end || //
        ...
        (arr[i].compareTo(arr[l]) >= 0 && arr[i].compareTo(arr[r]) >= 0)) return;
  ...
  public static <T extends Comparable<T>> int rotatedArrFind(T[] arr, T target) {
    ...
    while (l <= r) {
      ...
      if (arr[m].compareTo(arr[l]) > 0)
        if (target.compareTo(arr[l]) >= 0 && target.compareTo(arr[m]) < 0) return BS(arr, target, l, m - 1);
        else l = m + 1;
      else if (arr[m].compareTo(arr[l]) < 0) {
        if (target.compareTo(arr[m]) > 0 && target.compareTo(arr[r]) <= 0) return BS(arr, target, m + 1, r);
        else r = m - 1;
      } else l++;
  ...
  public static <T extends Comparable<T>> int BS(T[] arr, T target, int l, int r) {
    ...
    while (l <= r) {
      ...
      else if (target.compareTo(arr[m]) < 0) r = m - 1;
      else l = m + 1;
      m = (l + r) / 2;
    }
    return -1;
